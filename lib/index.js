...
class EventBus {
    ...
    constructor(options) {
        ...
        this._callbackCache = new LRU(cacheOptions);
    }
    ...
    safeUnsubscribe(resource, topic, handler) {
        const handlers = this._callbackCache.get(`${resource}.${topic}`);
        this._log.debug(`Looking for handler in cache`);
        if (handlers && handlers.length) {
            const index = handlers.findIndex(el => el.handler === handler);
            ...
            if (index >= 0) {
                ...
                this._callbackCache.set(`${resource}.${topic}`, handlers);
        ...
        this._log.error(
            `Handler ${handler.toString()} not exist!`
        )
    }
    ...
    #createCallback(resource, topic, handler) {
        ...
        if (!this._callbackCache.has(`${resource}.${topic}`)) {
            this._log.debug(
                `Created handlers array with ${JSON.stringify([{ callback: typeof callback, handler: typeof handler }])}`
            );
            this._callbackCache.set(`${resource}.${topic}`, [
                { callback, handler },
            ]);
        } else {
            this._log.debug(
                `Added callback to array ${JSON.stringify({ callback: typeof callback, handler: typeof handler })}`
            );
            const handlers = this._callbackCache.get(`${resource}.${topic}`);

            handlers.push({ callback, handler });

            this._callbackCache.set(
                `${resource}.${topic}`,
                handlers
            );
            this._log.warn(
                `Created unused handler for ${resource}.RESPONSE.${topic}`
            )
        }
        return callback;
    }
